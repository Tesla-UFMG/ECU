...
void rgb_led(void* argument) {
    ...
    for (;;) {
        ...
        switch (osMessageQueueGet(q_rgb_led_messageHandle, &message, NULL,
                                  RGB_BLINK_DELAY)) { // espera RTD ser setado ou timeout estourar

        case osErrorTimeout: // caso timeout estore vai piscar o led, indicando
                             // que ta fora do RTD
