...
void controle(void* argument) {
    ...
    // veloc_total = (speed_t_total[0] + speed_t_total[1] + speed_t_total[2] + speed_t_total[3]) /
    // 4;
    //  vehicle_state_e vehicle_state;

    // mensagem de referencia de torque. Contem qual valor de torque se deseja e se eh desejado
    // desabilitar
    ...
    for (;;) {
        ...
        bool disable;
        disable = !get_individual_flag(
            ECU_control_event_id, RTD_FLAG); // disable will only be FALSE when RTD_FLAG is setted
