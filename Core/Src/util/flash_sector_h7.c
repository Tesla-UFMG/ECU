	 ...
	 /* Unlock the Flash to enable the flash control register access *************/
	  ...
	  //Save
	   while (sofar<numberofwords)
	   {
	     ...
	     {
	    	 //Sector address goes from 32 to 32 positions, as each saved number is 256 bits length
...
//All this functions below would serve to save float numbers in the memory. As our use is only uint32's, I commented them below

		//uint8_t bytes_temp[32];
		...
		//
		//
		//	Flash_Write_Data (StartSectorAddress, (uint32_t *)bytes_temp, 8);
		//}
		//
		//
