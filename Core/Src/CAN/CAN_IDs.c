...
// TODO(renanmoreira): talvez aumentar capacidade se precisar de mais ids de
// debug
...
CAN_ID_t get_CAN_ID_from_internal(uint16_t internal) { return CAN_ID_map[internal]; }
...
void initialize_CAN_IDs() {
    ...
    CAN_ID_map[ID_DISTANCE_ODOM] = (CAN_ID_t)INTERNAL_ID_DISTANCE_ODOM;
    ...
    CAN_ID_map[ID_REF_SPEED_R] = (CAN_ID_t)INTERNAL_ID_REF_SPEED_R;
    CAN_ID_map[ID_REF_SPEED_L] = (CAN_ID_t)INTERNAL_ID_REF_SPEED_L;
    ...
    for (uint16_t i = 0; i < CAN_ID_QUAN; i++) {
        ...
        if (current_ID->id > ECU_CAN_LAST_POPULATED_ID) {
            unique_index = convert_id_pos_to_index(ECU_CAN_LAST_POPULATED_ID + 1 + current_ID->id -
                                                       ECU_CAN_FIRST_DEBUG_ID,
                                                   current_ID->pos);
        } else {
