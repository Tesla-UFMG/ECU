...
void rampa_torque_lateral( lateral_t *ref_torque_decrease, uint32_t *ref_torque) {
	...
	ref_torque[R_MOTOR] = (modo_selecionado.torq_gain * throttle_percent) / 10;
	ref_torque[L_MOTOR] = (modo_selecionado.torq_gain * throttle_percent) / 10;

	if (ref_torque_decrease->ref_wheel == R_MOTOR)
		ref_torque[R_MOTOR] -= (uint32_t)ref_torque_decrease->ref_decrease;
	else
		ref_torque[L_MOTOR] -= (uint32_t)ref_torque_decrease->ref_decrease;
