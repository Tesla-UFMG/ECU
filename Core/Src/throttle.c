...
void throttle_read(void *argument) {
    ...
    for (;;) {
        ...
        apps2_throttle_percent =
            calculate_apps2(APPS2); // calcula a porcentagem do pedal a partir do APPS2
        ...
        check_for_errors_with_timeout(is_there_SU_F_error, SU_F_ERROR_FLAG, tim_SU_F_errorHandle,
                                      SU_F_ERROR_TIMER);
...
bool is_there_APPS_error() { // Regulamento: T.4.2 (2021)
    if (APPS2 >= 3500        // Se o valor de APPS2 for acima do seu maximo
        ...
        || APPS1 < apps1_calc * (1 - APPS_PLAUSIBILITY_PERCENTAGE_TOLERANCE /
                                         100.0) // verifica se APPS1 esta abaixo do valor
                                                // teorico de APPS1, considerando a tolerancia
        || APPS1 > apps1_calc * (1 + APPS_PLAUSIBILITY_PERCENTAGE_TOLERANCE /
                                         100.0)) { // verifica se APPS1 esta acima do valor
                                                   // teorico de APPS1, considerando a tolerancia
        return true;
    }
    return false;
...
bool is_there_BSE_error() {
    ...
    return (apps2_throttle_percent > APPS_25_PERCENT &&
