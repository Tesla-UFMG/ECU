...
void speed_calc(void) {
    ...
    for (;;) {
        ...
        switch (osMessageQueueGet(
            q_speed_messageHandle, &message, NULL,
            min_timeout)) { // espera ate alguma mensagem chegar ou timeout estourar
        ...
        default:
            // verifica se alguma roda esta a muito tempo sem receber interrupcao, caso sim a
            // velocidade dessa roda eh zerada
            ...
            // caso d_tim_count calcule uma velocidade maior do que eh possivel o valor sera
            // descartado
...
// obtem a frequencia do tim2 a partir APB1, considerando que ele pode ter um prescaler que dobra a
// frequencia
static inline uint32_t get_tim2_freq() {
