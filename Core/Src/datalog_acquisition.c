...
void datalog_acquisition(){
	...
	for(;;){
		...
		flag_ecu_1 = (osEventFlagsGet(ECU_control_event_id) >> 16);     // valor da flag é passado e shiftado para a direita 16x
		flag_ecu_1 = flag_ecu_1 & 0xffff;                                               // apos ser shiftado o que permaneceu nos 16 bits mais significativos é
		flag_ecu_2 = (osEventFlagsGet(ECU_control_event_id) & 0xffff);  // ignorado a partir da operação and com os 16 bits menos significativos
