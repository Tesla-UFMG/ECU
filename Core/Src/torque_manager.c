...
void torque_manager(void *argument) {
    ...
    for (;;) {
        ...
        case LATERAL:
            ...
            void rampa_torque_lateral(lateral_t * ref_torque_decrease,
                                      uint32_t * ref_torque); // TODO(renanmoreira): utilizar
                                                              // rampa_torque enquanto controle
...
uint32_t rampa_torque() {
    ...
    uint32_t ref_torque = (uint32_t)((float)(get_global_var_value(SELECTED_MODE).torq_gain *
...
void rampa_torque_lateral(lateral_t *ref_torque_decrease, uint32_t *ref_torque) {
    ...
    ref_torque[R_MOTOR] = (uint32_t)((float)(get_global_var_value(SELECTED_MODE).torq_gain *
                                             get_global_var_value(THROTTLE_PERCENT)) /
                                     10);
    ref_torque[L_MOTOR] = (uint32_t)((float)(get_global_var_value(SELECTED_MODE).torq_gain *
                                             get_global_var_value(THROTTLE_PERCENT)) /
                                     10);

    // A referencia de torque sera a referencia acima menos a referencia de
    // decrescimento. Se chegar a valores negativos, entao sera 0.
    ref_torque[ref_torque_decrease->ref_wheel] = (uint32_t)max(
        (int32_t)(ref_torque[ref_torque_decrease->ref_wheel] - ref_torque_decrease->ref_decrease),
        0);
    ...
    for (int i = 0; i < 2; i++) {
        if (ref_torque_ant[i] > TORQUE_INIT_LIMITE) {             // verifica se a referencia
            if (ref_torque[i] > ref_torque_ant[i] + INC_TORQUE) { // ja passou do ponto de inflexao
                ref_torque[i] =
                    ref_torque_ant[i] + INC_TORQUE; // e aplica o incremento mais agressivo
            }
        } else {
