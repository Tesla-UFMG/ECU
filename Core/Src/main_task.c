...
void main_task(void* argument) {
    ...
    for (;;) {
        ...
        case INVERTER_BUS_OFF_ERROR_FLAG:
            isErrorPresent =
                event_flags &
                INVERTER_BUS_OFF_ERROR_FLAG; // verifica se o erro esta presente na flag de evento
            if (isErrorPresent) {
                exit_RTD(); // sai de RTD caso o erro esteja presente
            } else {        // caso o erro nao esteja ainda:
                osEventFlagsSet(ECU_control_event_id,
                                INVERTER_BUS_OFF_ERROR_FLAG); // seta a flag de evento para que caso
                                                              // tenha erro novamente saia de RTD
        ...
        case INVERTER_COMM_ERROR_FLAG:
            ...
            isErrorPresent =
                event_flags & INVERTER_COMM_ERROR_FLAG; // verifica se o erro ainda esta presente na
                                                        // flag de evento
            if (isErrorPresent) {
                exit_RTD(); // sai de RTD caso o erro esteja presente
        ...
        case SU_F_ERROR_FLAG:
            isErrorPresent =
                event_flags &
                SU_F_ERROR_FLAG; // verifica se o erro ainda esta presente na flag de evento
            if (isErrorPresent) {
                exit_RTD();                          // sai de RTD caso o erro esteja presente
            } else {                                 // caso o erro tenha sido resolvido:
        ...
        case APPS_ERROR_FLAG: // Regulamento: T.4.2 (2021)
            isErrorPresent =
                event_flags &
                APPS_ERROR_FLAG;  // verifica se o erro ainda esta presente na flag de evento
        ...
        case BSE_ERROR_FLAG: // Regulamento: EV.5.7 (2021)
            isErrorPresent =
                event_flags &
                BSE_ERROR_FLAG;   // verifica se o erro ainda esta presente na flag de evento
            if (isErrorPresent) { // caso o erro esteja presente:
                set_rgb_led(AMARELO, NO_CHANGE); // seta o led rgb como amarelo
                osDelay(20);
            } else {                                // caso o erro tenha sido resolvido:
