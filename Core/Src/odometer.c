...
void odometer_calc(void *argument) {
    ...
    // verifica se o calculo da distancia ja deu 1 metro ou mais - valor
    retornado em dist_calc esta em decimetro

    if ((dist_calc / 10) >= 1) {
            dist_log[TEST_DIST] += (dist_calc / 10);		// updates test and
    overall distance
            dist_log[OVERALL_DIST] += (dist_calc / 10);		// updates
    distance already travelled

            // rounds to the nearest meter
            Record_Distance(dist_log);						// writes values
    in flash memory
    }*/
}
