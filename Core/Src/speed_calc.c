...
// tempo         = valor do timer multiplicado pelo prescaler dividido pela
// frequencia
...
void speed_calc(void) {
    ...
    // NOLINTNEXTLINE
    memset(&last_messages, 0,
           sizeof(speed_message_t) * 4); // inicializa com 0 buffer de ultimas mensagens
    ...
    for (;;) {
        ...
        switch (osMessageQueueGet(q_speed_messageHandle, &message, NULL,
                                  min_timeout)) { // espera ate alguma mensagem
                                                  // chegar ou timeout estourar
        ...
        default:
            // verifica se alguma roda esta a muito tempo sem receber
            // interrupcao, caso sim a velocidade dessa roda eh zerada
            ...
            // caso d_tim_count calcule uma velocidade maior do que eh possivel
            // o valor sera descartado
            if (d_tim_count < max_count) {
                continue;
            }

            speed = calculate_speed(d_tim_count, tim_freq,
                                    tim_presc); // calcula a velocidade
            WHEEL_SPEEDS_t wheel_speeds = get_global_var_value(WHEEL_SPEEDS);
            wheel_speeds.speed[message.pin] =
                (float)speed; // seta velocidade especifica da roda recebida
...
// obtem a frequencia do tim2 a partir APB1, considerando que ele pode ter um
// prescaler que dobra a frequencia
static inline uint32_t get_tim2_freq() {
    if (RCC->D2CFGR & RCC_D2CFGR_D2PPRE1) { // Get PCLK1 prescaler
        return 2 * HAL_RCC_GetPCLK1Freq();  // PCLK1 prescaler different from 1
                                            // => TIMCLK = 2 * PCLK1
    }
    return HAL_RCC_GetPCLK1Freq(); // PCLK1 prescaler equal to 1 => TIMCLK =
                                   // PCLK1
}
...
static inline uint32_t calculate_timeout(uint32_t speed) {
    return (uint32_t)((10 * 3.6 * 2 * M_PI * WHEEL_RADIUS / SPEED_SENSOR_TEETH_QUAN) * 1000 /
                      speed);
}
