...
lateral_result_t lateral_control() {
    ...
    GYRO_YAW_t gyro_yaw =
        get_global_var_value(GYRO_YAW); // TODO(renanmoreira): receber GYRO_YAW em algum lugar
    ...
    // speed
    cg_speed = avg(wheel_speeds.speed[FRONT_RIGHT], wheel_speeds.speed[FRONT_LEFT]) /
               (10 * 3.6); // velocidade em m/s
    // steering
    ...
    setpoint = fabs(desired_yaw) > fabs(max_yaw)
                   ? max_yaw
                   : desired_yaw; // o menor valor, em modulo, sera o setpoint
    // PID
    ...
    if (pid_result > 0) { // se o sinal for positivo, a reducao sera
        ref_torque_result.torque_decrease[R_MOTOR] = fabs(pid_result); // na roda direita
    } else {                                                           // caso contrario, sera
        ref_torque_result.torque_decrease[L_MOTOR] = fabs(pid_result); // na roda esquerda
    }
...
float calc_steering(uint16_t steering_wheel,
                    uint8_t internal_wheel) { // TODO(renanmoreira): verificar valor do steering
