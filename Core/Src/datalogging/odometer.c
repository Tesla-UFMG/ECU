...

{
    ...
    if ((dist_traveled.distances[TOTAL_DIST] == 0x00000000)
        || (dist_traveled.distances[TOTAL_DIST] == 0xFFFFFFFF)) {
    ...
    for (;;) {
        ...
        // If the distance is littler than 100 meters send distance data every meter
        // traveled
        if (dist_traveled.distances[PARTIAL_DIST] < 100) {
            ...
            // After the distance surpass 100 meters, send partial distance data only when
            // other 100 meters are traveled
