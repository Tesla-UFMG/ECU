...
uint16_t calculate_apps2(uint16_t APPS2) {
    uint16_t value[APPS_MATRIX_LENGTH] = {APPS2_REF};
    float fix_mul[APPS_MATRIX_LENGTH] = {APPS2_FIX_MUL};
    float fix_add[APPS_MATRIX_LENGTH] = {APPS2_FIX_ADD};
    uint16_t percentage = 0;

    for(int i=0; i<APPS_MATRIX_LENGTH; i++) {
        if(APPS2 < value[i]) {
            percentage = (fix_mul[i] * APPS2 + fix_add[i]);
            break;
        }
    }
    if (percentage > 1000)
...
uint16_t calculate_expected_apps1_from_apps2(uint16_t apps2_percentage) {
    uint16_t value[APPS_MATRIX_LENGTH] = {APPS1_REF};
    float fix_mul[APPS_MATRIX_LENGTH] = {APPS1_FIX_MUL};
    float fix_add[APPS_MATRIX_LENGTH] = {APPS1_FIX_ADD};
    ...
    if(apps2_percentage < 0 || apps2_percentage > value[8])
        ...
        for(int i=0; i<APPS_MATRIX_LENGTH; i++) {
            if(apps2_percentage < value[i]) {
                return (fix_mul[i] * apps2_percentage + fix_add[i]);
            }
        }
    }
    return 0;
