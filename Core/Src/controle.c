...
void update_state(bool disable) {
    ...
    } else if ((get_global_var_value(THROTTLE_PERCENT) < 100) && (frenagem_regenerativa == true) &&
               get_global_var_value(MOTOR_SPEEDS).speed[L_MOTOR] > VAL_5_kmph_rpm) {
...
void update_state_parameters(torque_message_t* torque_message) {
    ...
    case S_NEUTER_E:
        ...
        // TODO(renanmoreira): mudar velocidade do motor de acordo com nova
        // logica
        MOTOR_SPEEDS_t motor_speeds = get_global_var_value(MOTOR_SPEEDS);
        set_bit8(&torque_message->parameters, P_RUNSTOP,
                 (motor_speeds.speed[R_MOTOR] > VAL_5_kmph_rpm ||
                  motor_speeds.speed[L_MOTOR] > VAL_5_kmph_rpm));
    ...
    case S_ACCELERATE_E:
        ...
        set_bit8(&torque_message->parameters, P_BRAKE, false);
        set_bit8(&torque_message->parameters, P_RUNSTOP, true);
        // TODO(renanmoreira): Mudar para nova logica de envio de mensagem de
        // torque ao inversor
        torque_message->neg_torque_ref[R_MOTOR] = 0;
        torque_message->neg_torque_ref[L_MOTOR] = 0;
        torque_message->speed_ref[R_MOTOR] = selected_mode.vel_max;
...
void controle(void* argument) {
    ...
    // mensagem de referencia de torque. Contem qual valor de torque se deseja e
    // se eh desejado desabilitar
    ...
    for (;;) {
        ...
        bool disable;
        disable =
            !get_individual_flag(ECU_control_event_id,
                                 RTD_FLAG); // disable will only be FALSE when RTD_FLAG is setted

        // getflag
        switch (
            osMessageQueueGet(q_ref_torque_messageHandle, &ref_torque_message, 0, CONTROLE_DELAY)) {
...
void update_regen_state(vehicle_state_e vehicle_state) {
    if (vehicle_state == S_BRAKE_E) {
        osEventFlagsSet(ECU_control_event_id,
                        REGEN_WARN_FLAG); // se frenagem ativa, seta flag de aviso
    } else {
        osEventFlagsClear(ECU_control_event_id,
                          REGEN_WARN_FLAG); // se frenagem ativa, limpa flag de aviso
    }
}
