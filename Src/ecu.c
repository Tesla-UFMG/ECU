...
void actual_datalogger() {
	if (flag_can == 1) //variÃ¯Â¿Â½vel "flag_dtl" assume o valor "2" no final da main
	{
...
void torque_vectoring (void)
{
		uint16_t vol_gain_externo, vol_gain_interno, vel_diant_ext = 0 ;
			double const0=0.5, const1=-0.0005221, const2=0.000005772, const3= 0.00009419; //constatntes baseadas em simualaï¿½ï¿½o
			...
			if(roda_interna == RODA_DIANT_DIR)	// curva para a direita
			{
				...
				vol_gain_externo = (const0 + const1*volante*vel_diant_ext + const2*volante*volante*vel_diant_ext + const3*volante*vel_diant_ext*vel_diant_ext) * 100;//equaï¿½ï¿½o baseada em simulaï¿½ï¿½o de transferencia de peso
