...
typedef dynamic_control_t DYNAMIC_CONTROL_t;
 ...
 * }
 *
 * This means the macros can't be used as 1:1 replacement for functions, as they are
 * simply rules for code substitution. Care must be taken as to where to call the macros,
 * as the substitution can lead to undefined behavior.
 */
 ...
 * \return void
 * \example
 * if called like 'set_global_var_value(THROTTLE_PERCENT, 100)' - i.e.
 ...
 *                                });
 *
 * The '({})' block is a Statement Expression, from GNU C. It allows declarations and
 * statements within an expression, being the last expression-like statement the "return"
 * value of the expression. This acts like an anonymous execution block, avoid the
 * pollution of the scope in which the macro was invoked.
 */
