...
void initialize_CAN_IDs() {
	...
	CAN_ID_map[ID_REF_TORQUE_NEG] 		= (CAN_ID_t)INTERNAL_ID_REF_TORQUE_NEG;
	CAN_ID_map[ID_REGEN_BRAKE_QUAN] 	= (CAN_ID_t)INTERNAL_ID_REGEN_BRAKE_QUAN;
	...
	for (uint16_t i = 0; i < CAN_ID_QUAN; i++) {
		...
		if (current_ID->id > ECU_CAN_LAST_POPULATED_ID) {
			unique_index = convert_id_pos_to_index(ECU_CAN_LAST_POPULATED_ID+1+current_ID->id-ECU_CAN_FIRST_DEBUG_ID, current_ID->pos);
		} else {
