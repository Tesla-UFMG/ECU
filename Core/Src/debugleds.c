...
void debugleds(void* argument) {
    ...
    for (;;) {
        ...
        osMessageQueueGet(q_debugleds_messageHandle, &message, NULL,
                          osWaitForever); // espera ate alguma mensagem chegar
        switch (message.control) {
        case ON:
            HAL_GPIO_WritePin(GPIOE, message.lednumber,
                              GPIO_PIN_RESET); // acende o led escolhido
            break;
        case OFF:
            HAL_GPIO_WritePin(GPIOE, message.lednumber,
                              GPIO_PIN_SET); // apaga o led escolhido
            break;
        case TOGGLE:
            HAL_GPIO_TogglePin(GPIOE,
                               message.lednumber); // toggle o led escolhido
            break;
        case BLINK:
            for (uint8_t i = 0; i < message.amount;
                 i++) { // pisca o led escolhido por quantas vezes selecionadas
                HAL_GPIO_WritePin(GPIOE, message.lednumber,
                                  GPIO_PIN_RESET); // delays garante que eh possivel contar
                                                   // quantas vezes piscou
                osDelay(300);
